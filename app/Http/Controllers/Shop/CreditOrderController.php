...
class CreditOrderController extends Controller
{
    ...
    {
        $value = $request->all();
        $list = ShopOrder::orderList($value,1);
        return view('shop.creditOrder.index')->with(['datas'=>$list,'value'=>$value]);
    }
    ...
    public function ship(Request $request,$id){
        ...
        $order->ex = $a;
        return view('shop.creditOrder.ship', compact('order',$order));
    }
...
//        echo $order;exit;
        return view('shop.creditOrder.detail', compact('order',$order));
    }
    public function updateExpress(Request $request) {
        ...
        if($validator->fails()){
            return redirect("shop/creditOrder")->withErrors($validator->messages()->first());
        }
        $params['remark'] = isset($params['remark']) ? $params['remark'] : "";
        $order = ShopOrder::editCreditShip($params);
        return redirect("shop/creditOrder")->with('flash_message','提交成功');
    }
...
//        dd($url);
        ...
        if($ex_info == null){
            return redirect("shop/creditOrder")->withErrors('物流信息错误');
        }
        $kd = json_decode($kd_info);
        if($kd->Success == false){
            return view('shop.creditOrder.kd')->withErrors($kd->Reason);
        }
        
        $kd->Traces = array_reverse($kd->Traces);
        return view('shop.creditOrder.kd')->with(['order'=>$order,'kd'=>$kd,'ex'=>$ex_info]);
