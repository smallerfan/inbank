...
class CoinincreategemsController extends Controller
{
    public function index()
    {
        $datas = Coin::query()->where('plan_type',1)->with('coin_item')
            ->paginate(10);
        foreach ($datas as $k => $v){
            $v->support_coins = explode(',',$v->support_coins);
        }
//        dd($datas[0]->support_coins [0]);
        return view('gem.coin.index', ['datas'=>$datas]);
    }

    public function create()
    {
        $coin_type = C2cCoin::all();
        return view('gem.coin.create')->with(['coin'=>$coin_type]);
    }
    ...
    public function add(Request $request)
    {
        ...
        if($validator->fails()){
        return redirect("gem/coin/create")->withErrors($validator->messages()->first());
    }
...
//                print_r($v);
            ...
            DB::commit();
            return redirect()->route("coin.index")->with('flash_message','添加成功');

        }catch (\Exception $exception){
            DB::rollBack();
            return redirect()->route("coin.index")->withErrors('添加失败');
        }
    }
    ...
    public function edit($id)
    {
        ...
        $coin_type = C2cCoin::all();
        return view('gem.coin.edit',['data'=>$data,'coin'=>$coin_type]);
    }
...
//        dd($params);
        ...
        if($validator->fails()){
            return view("gem.coin.edit_plan")->withErrors($validator->messages()->first());
        }
        foreach ($params['support_coins'] as $k => $v){
            $a[] = $k;
        }
        ...
        if($params['support_coins'] !== $plan->support_coins){
            ...
            DB::commit();
            return redirect()->route("coin.index")->with('flash_message','修改成功');

        }catch (\Exception $exception){
            DB::rollBack();
            return redirect()->route("coin.index")->withErrors('修改失败');
        }
    }
    ...
    public function destroy($id)
    {
        ...
        if($coin){
            ...
            return redirect()->route("coin.index")->with('flash_message','删除成功');
        }else{
            return redirect()->route("coin.index")->withErrors('删除失败');
        }
    }
    ...
    public function set_open($id)
    {
        ...
        if($coin){
            $coin->status='1';
            $coin->save();
            return redirect()->route("coin.index")->with('flash_message','正常成功');
        }else{
            return redirect()->route("coin.index")->with('flash_message','正常不成功');
        }
    }
    ...
    public function set_close($id)
    {
        ...
        if($coin){
            $coin->status='0';
            $coin->save();
            return redirect()->route("coin.index")->with('flash_message','禁用成功');
        }else{
            return redirect()->route("coin.index")->with('flash_message','禁用失败');
        }
    }
}
