        ...
        <div class="page-header">
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">充值列表</li>
                </ol>
            </nav>
        </div>
        <div class="row">
            <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title">充值列表</h4>
                        <div>
                            <ul>
                                @foreach($coin as $c)
                                    @if($c->sys_name == 'ETH')
                                        <li id="eth" @if($msg == 'ETH' || empty($msg)) class="active" @endif><a href="{{ route('recharge.index') }}" >ETH充值</a></li>
                                    @else
                                        <li id="{{strtolower($c->sys_name)}}" @if($msg == $c->sys_name || empty($msg)) class="active" @endif><a href="{{ route('recharge.lists',$flag = $c->sys_name) }}" >{{$c->sys_name}}充值</a></li>
                                    @endif
                                @endforeach
                            </ul>
                        </div>
                        {{ Form::open(['route' => 'recharge.index', 'method' => 'get','class'=>'form-inline']) }}
                        <input type="hidden" name="coin" value="{{ $msg }}" id="coin" class="form-control">
                        <select name="status" class="form-control" >
                            <option value='' @if(empty($status)) selected @endif>充值状态</option>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="btn-group" role="group" aria-label="Button group with nested dropdown">
                                                <a href="{{ route('recharge.edit', $data) }}" class="btn btn-gradient-info btn-sm" style="margin-right: 3px;">
